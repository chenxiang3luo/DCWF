...
class SnapshotModelCheckpoint(Callback):
    ...
    def __init__(self,bf, nb_epochs, nb_snapshots, fn_prefix='Model'):
        super(SnapshotModelCheckpoint, self).__init__()
        self.bf = bf
        self.check = (nb_epochs -bf) // nb_snapshots
        print("check is ",self.check)
        self.fn_prefix = fn_prefix

    def on_epoch_end(self, epoch, logs={}):
        if epoch != 0 and (epoch + 1) % self.check == 0 and epoch>self.bf:
...
class SnapshotCallbackBuilder:
    ...
    def __init__(self,bf,nb_epochs, nb_snapshots, init_lr=0.1):
        """
    ...
    def get_callbacks(self, model_prefix='Model'):
        ...
        callback_list = [callbacks.ModelCheckpoint("weights/%s-Best.h5" % model_prefix, monitor="val_acc",
                         ...
                         SnapshotModelCheckpoint(self.bf,self.T, self.M, fn_prefix='weights/%s' % model_prefix)]
