...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes,kernel_size = 8,stride=1,act = 0):
        ...
        self.conv1 = nn.Conv1d(inplanes, planes, kernel_size=kernel_size,stride=stride,padding='same')
        self.bn1 = nn.BatchNorm1d(planes)
        if act == 0:
            self.elu = nn.ELU(inplace=True)
        else:
            self.elu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv1d(planes, planes, kernel_size=kernel_size,stride=stride,padding='same')
